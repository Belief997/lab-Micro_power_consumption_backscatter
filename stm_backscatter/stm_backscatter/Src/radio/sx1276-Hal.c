...
#define SENSOT_IOPORT 																	GPIOB
#define SENSOT_PIN																			GPIO_PIN_4
...
void tri_sensor(void){
	uint32_t tickstart;
	GPIO_WriteBit(SENSOT_IOPORT, SENSOT_PIN,Bit_RESET);
	tickstart = HAL_GetTick();
	while(HAL_GetTick()-tickstart<=2);
	GPIO_WriteBit(SENSOT_IOPORT, SENSOT_PIN,Bit_SET);
}
void toggle_sensor(void){
	HAL_GPIO_TogglePin( SENSOT_IOPORT, SENSOT_PIN);
}

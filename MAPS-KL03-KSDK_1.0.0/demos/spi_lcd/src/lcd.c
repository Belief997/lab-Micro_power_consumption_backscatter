...
static void LCD_Spi_Send(unsigned char value)
{
    ...
    status = SPI_DRV_MasterTransferBlocking(0, NULL, &value, NULL, 1, OSA_WAIT_FOREVER); 
    if(status != kStatus_SPI_Success)
    {
        printf("ERROR: [LCD_Spi_Send]SPI transfer(1 byte) failed, error code: %d\r\n", status);
    }
}
...
unsigned char  LCD_PutChar(unsigned long x, unsigned long y, unsigned char ch)
{
    ...
    for(i = 0; i < FONT_VSIZE; i++)
    {
        ...
        for(j = 0; j < FONT_HSIZE; j++)
        {
            ...
            if(sUpdate_mode != 0) 
            {
                LCD_UpdatePoint(x, y, write_data);
            }
            x ++;
        }
        x -= FONT_HSIZE;								
